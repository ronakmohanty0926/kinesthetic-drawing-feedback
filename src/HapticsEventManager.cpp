...
{	
	...
	//float xRng[] = { -212.21,213.18 }; float yRng[] = { -107.35, 190.732 }; float zRng[] = { -50.0, 94.42 };
	...
	Normalize3(displacement, _disp);

	//float planePt[3]; skAPI->SetPlaneCenter(planePt);

	float newOrigin[] = { 0.02,-0.31,0.35 }; //GL Coordinates
	float planeNorm[] = { Zaxis[0], Zaxis[1], Zaxis[2] };

	float degAngle = -1*(skAPI->planeAngle); 
	float radAngle = (PI / 180)*(degAngle);
	cerr << "angle->" << 90 - (-1*degAngle) << endl;
	AxisAngle(Xaxis, radAngle, matrix);
	RotateVec3(planeNorm, matrix);
	/*Transformation t;
	float transl[] = { 0.02, -0.31, 0.088 };
	t.SetTranslationBy(transl);
	t.ApplyTo(Norm);*/
	Normalize3(planeNorm);

	//cerr << "Normal" << Norm[0] << " " << Norm[1] << " " << Norm[2] << endl;

	float planeDist = PointToPlaneDist(stylus, newOrigin, planeNorm);
	float displ[3];

	//cerr << "Distance->" << planeDist << endl;
	...
	if (planeDist < 0.1 && stylus[0] > -0.80 && stylus[0] < 0.80 && stylus[1] < 0.5 && stylus[1] > -0.5 && isSketchOn)
	{
		float stiffness = 15.0;
